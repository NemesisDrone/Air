...
class Vl53Component(component.Component):
    ...
    def _init_i2c(self):
        ...
        try:
            ...
            self.logger.warning(
                f"Could not initialize I2C bus for VL53, defaulting to emulated data for both sensors: {e}",
                self.NAME
            )
    ...
    def _init_vl53(self, number: int):
        ...
        """
        pin, label, address = (self._first_sensor_shut_pin, "first", 0x30) if number == 1 else (
            self._second_sensor_shut_pin, "second", 0x29)
        ...
        try:
            ...
            self.logger.warning(
                f"Could not initialize {label} sensor, defaulting to emulated data for this sensor: {e}",
                self.NAME
            )
    ...
    def _update_custom_status(self):
        ...
        self.redis.set(
            "sensors:vl53:status",
            json.dumps({"sensing_worker_alive": self._sensing_worker_alive,
