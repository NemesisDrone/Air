...
class Manager:
    def __init__(self, ipc_node: ipc.IpcNode):
        ...
        self._components: (typing.Dict)[
            str, typing.Dict[str, typing.Union[multiprocessing.Process, None, threading.Lock]]
    ...
    def _timout_state_update(self, component: str):
        ...
        if r:
            ...
            self._ipc_node.logger.error(
                f"Timout reached for {component} component witch is still {state}, " f"killing process.", "manager"
            )
            ...
            self._ipc_node.send(
                f"state:{component}:{component_module.ComponentState.STOPPED}", {"component": component}, loopback=True
            )
            self._ipc_node.logger.info(f"component is {component_module.ComponentState.STOPPED}", component, "state")
    ...
    def _start_component(self, component: str):
        ...
        self._components[component]["process"] = multiprocessing.Process(
            target=component_module.run_component, args=(components[component],)
        )
...
if __name__ == "__main__":
    ...
    init_profile = (
        "default"
        if (os.environ.get("NEMESIS_PROFILE") == "" or os.environ.get("NEMESIS_PROFILE") is None)
